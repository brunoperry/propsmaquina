...
export default class GQuaternion {
  constructor(data) {
    if (!data) {
      this.x = 0;
      this.y = 0;
    ...
    switch (data.type) {
      case GQuaternion.Type.INIT_VALS:
        this.constructorWithVals(data.x, data.y, data.z, data.w);
        break;
      case GQuaternion.Type.INIT_VEC_ANGLE:
        this.constructorWithVecAngle(data.vec, data.angle);

        break;
      case GQuaternion.Type.INIT_MAT:
  ...
  conjugate() {
    return new GQuaternion({
      type: GQuaternion.Type.INIT_VALS,
      x: -this.x,
      y: -this.y,
      z: -this.z,
      w: this.w,
  ...
  static sLerp(a, b, t, shortest = false) {
    ...
    if (Math.abs(cos) >= 1 - EPSILON)
      return GQuaternion.nLerp(a, correctedDest, t, false);
  ...
  SLerp(dest, lerpFactor, shortest = false) {
    ...
    return this.multiplyF(srcFactor).addQuat(
      correctedDest.multiplyF(destFactor)
    );
  }
