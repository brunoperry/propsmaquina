class Bitmap {
  constructor(data) {
    ...
    this.name = data.name;
    data.name ? (this.name = data.name) : (this.name = "no_name");
    this.mode = data.mode;
    data.mode ? (this.mode = data.mode) : (this.mode = Bitmap.Modes.OPAQUE);
    ...
    if (src.mode === Bitmap.Modes.TRANSPARENT) {
      ...
      if (aValue > 0) {
        color = Color.blendAlpha(
          ...
          new Color(
            this.components[destIndex],
            this.components[destIndex + 1],
            this.components[destIndex + 2]
          ),
